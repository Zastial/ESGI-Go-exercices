...
## Prochaines étapes (Pour optimiser le barème)

1. **Ajouter les tests** (4 points complets)
   ```bash
   # Tests handlers
   cd tp4
...
2. **Configurer GitHub Actions** (3 points)
   ```bash
   mkdir -p .github/workflows
   # Créer workflow pour : lint, test, build
   ```

3. **Enregistrer une vidéo de démo** (Bonus)
   - CLI : `mira add`, `mira list`, `mira search`
   - API : Tester les endpoints via curl
   - MCP : Utiliser les tools dans Claude Code

## Ressources

- **Go 1.26** : https://golang.org/doc/go1.26
- **pgx** (PostgreSQL driver) : https://github.com/jackc/pgx
- **pgvector** (Vector embeddings) : https://github.com/pgvector/pgvector
- **MCP Protocol** : https://modelcontextprotocol.io/

## Support

En cas de problème :
- Vérifier que PostgreSQL tourne : `docker ps | grep postgres`
- Vérifier l'API : `curl http://localhost:8080/api/v1/notes`
- Vérifier les logs de l'API : Console où l'API tourne
- Pour MCP : Vérifier `MIRA_API_URL` est défini
